	...
	case PHYSICAL_IN_EVENT: 
		{
			...
			else
			{
				...
				if(pstruEventDetails->nDeviceId == pstruEventDetails->pPacket->nDestinationId && pstruEventDetails->pPacket->nPacketType == PacketType_Custom)
...
_declspec(dllexport) int fn_NetSim_Zigbee_Metrics(char* szMetrics)
{
	...
	fprintf(fp,"#DATA METRICS\n");
	fprintf(fp,"DeviceID\tData Sent(Physical Layer)\tData Received(Physical Layer)\tLifetime(s)\n");
	
	for(i=0; i<NETWORK->nDeviceCount; i++)
	{
		fprintf(fp,"%d\t%d\t%d\t%f\n",NETWORK->ppstruDeviceList[i]->nDeviceId,Data_Packet_Sent_PL[NETWORK->ppstruDeviceList[i]->nDeviceId-1],Data_Packet_Received_PL[NETWORK->ppstruDeviceList[i]->nDeviceId-1],Lifetime[NETWORK->ppstruDeviceList[i]->nDeviceId-1]/1000000);
	}
	fclose(fp);                     
