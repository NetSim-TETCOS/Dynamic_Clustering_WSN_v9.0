...
_declspec(dllexport) int fn_NetSim_DSR_Run()
{
		...
		//n=(int*)realloc(n,(count+1)*sizeof*(n));
	//ClusterElements[0]=(int*)calloc(16,sizeof*(ClusterElements[0]));
	//ClusterElements[0]=(int*)realloc(ClusterElements[0],5,sizeof*(ClusterElements[0]));
...
_declspec(dllexport) int fn_NetSim_DSR_Metrics(char* filename)
{
	...
	//To enable Dynamic Clustering, uncomment the line "#define _DYNAMIC_CLUSTERING" in DSR.h
    	...
    	FILE* fp;
     	int i;
      	fp=fopen(filename,"a+");
       	fprintf(fp,"#Dynamic Clustering Metrics\n");
